...
export const ListDays = ({
  ...
  const [IndexDaySelected, setIndexDaySelected] =
    useState<number>(indexDaySelected);
        ...
        {days?.map((day, index) => {
          ...
          return (
            <button
              key={index}
              className={`
                flex-shrink-0 flex-col w-[98px] md:w-[80px] h-fit min-h-[72px] ${
                  !isEmptyWeek && day.selected
                    ? "bg-indigo-500"
                    : "bg-white border-2 w-[96px] border-indigo-500 "
                } rounded-lg text-center hover:cursor-pointer hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-indigo-500
                `}
              onClick={() => {
            ...
            >
              <div className="px-3 py-3">
                <p
                  className={`${
                    !day.selected || isEmptyWeek
                      ? "text-indigo-500"
                      : "text-white"
                  } $font-light text-sm`}
                >
                  {capitalize(day.name.slice(0, 3))}
                </p>
                <p
                  className={`${
                    !day.selected || isEmptyWeek
                      ? "text-indigo-500"
                      : "text-white"
                  } font-light pb-1 text-sm`}
                >
                  {dayjs(dayDate).format("DD")} {capitalize(month.slice(0, 3))}
                </p>
              </div>
            </button>
          );
