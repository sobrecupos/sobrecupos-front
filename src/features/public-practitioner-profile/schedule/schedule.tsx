...
export const Schedule = ({
  ...
  const [selectScheduleDay, setSelectScheduleDay] =
    useState<AppointmentsByPractice>();
  ...
  const selectDay = async (day: string, firstRender: boolean = false) => {
    ...
    const schedulePerDay = await appointmentsClient.getScheduleByDate({
      practitionerId,
      from,
    });
  ...
  const getNextDayWithAppointments = async (
    schedule: AppointmentsByPractice
  ) => {
    setIsLoading(true);
    ...
    const scheduleSort = await schedule.results.sort((a, b) => {
      return dayjs(a.appointments[0].start).diff(
        dayjs(b.appointments[0].start)
      );
    });
    const indexDayOfWeek = dayjs(
      scheduleSort[0].appointments[0].start.split("T")[0]
    ).day();

    setSelectedDate(
      dayjs(scheduleSort[0].appointments[0].start.split("T")[0]).format(
        "YYYY-MM-DDTHH:mm:ss.SSS"
      )
    );
    const updatedDays = Days.map((day, i) => ({
      ...day,
      selected: i === indexDayOfWeek + 1,
    }));
    setDays(updatedDays);
    ...
    if (schedule) {
      appointmentsClient
        ...
        .then((res) => {
          ...
          setEmptyWeek(false);
          setFirstNextDay({
            day: dayjs(scheduleSort[0]?.appointments[0].start.split("T")[0])
              .locale(localeEs)
          ...
          selectDay(dayjs(from).format("YYYY-MM-DDTHH:mm:ss.SSS"), true);
          return res;
        })
        .catch((err) => {
  ...
  if (hasError) {
    console.error(
      `schedule error: practitionerId: ${practitionerId}, practitioner: ${practitioner}, from: ${from}, to: ${to}`
    );
    return (
      <div className={classes.namespace} id="sobrecupos">
        ...
        <div className="mx-8 md:mx-auto md:w-2/4 bg-indigo-50 rounded-md p-2">
          <p className="text-lg md:text-sm text-center text">
            Si el error persiste, porfavor escribenos a{" "}
            <a
    ...
    <div className={`${classes.namespace}`} id="sobrecupos">
      {selected ? (
        ...
        <>
          ...
          <div>
            <p className={` text-left text-lg mb-5 font-medium  `}>
              {SelectedDate
                ? formatDate(SelectedDate)
                : formatDate(dayjs().format("YYYY-MM-DDTHH:mm:ss.SSS"))}
            </p>
          </div>
          <div></div>
          <div className="flex">
          ...
          selectScheduleDay &&
          selectScheduleDay?.results?.length <= 0 &&
          !showSpinner &&
          !isLoading ? (
            <div className={`${classes.empty}`}>
              ...
              <button
                className=" border-2 rounded-full py-4 px-5 border-indigo-500 font-bold text-indigo-600"
                onClick={() => getNextDayWithAppointments(schedule)}
              >
                Ir a hora más cercana
              </button>
            </div>
          ) : null}
          ...
          {!isLoading ? (
            ...
            <div
              className={`flex justify-center items-center text-indigo-500  min-h-[300px] relative `}
            >
